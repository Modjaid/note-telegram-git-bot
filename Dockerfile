...
# tzdata: IANA timezones for Node Intl (region.json / daily note local times)
RUN apk add --no-cache git tzdata

WORKDIR /app

COPY package.json package-lock.json LLmModels.json ./
RUN npm ci --omit=dev
...
COPY dist/note-log ./dist/note-log
COPY dist/rag ./dist/rag
